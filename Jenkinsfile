            ...
            docker.image('androidsdk/android-24:latest').inside('-u root') //Ссылка на образ: https://hub.docker.com/r/androidsdk/android-30
                    {
                        ...
                        stage('Run environment') {
                            ...
                            sh 'emulator -avd Android-22_google_apis_x86_64 -debug all -no-audio -no-window -gpu swiftshader_indirect -verbose -accel off&'
                            //работает без hardware на локале (Win64)
